...
namespace Hoteleria
{
    public partial class Pedido : Form
    {
        ...
        public void mostrar_pedido()
        {
            try
            {
                da = new OdbcDataAdapter("select P.idPedido as No, P.fechaPedido as Fecha, M.noMesa as Mesa, concat(C.nombreCliente,' ',C.apellidoCliente) as Cliente, c.nitCliente as Nit from pedido p inner join Cliente C on P.cliente_idCliente = C.idCliente inner join mesa m on p.mesa_idMesa=m.idMesa", con.conexion());
                dt = new DataTable();
                da.Fill(dt);
        ...
        private void dgv_pedido_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                com = new OdbcCommand("select p.idPedido, sum((pl.costoPlatillo*dpp.cantidadPlatillo+pro.precio* dpb.cantidadBebida)/2) as Costo from platillo pl inner join receta r on pl.receta_idReceta = r.idReceta inner join detallepedidoplatillo dpp on dpp.platillo_idPlatillo = pl.idPlatillo inner join pedido p on p.idPedido = dpp.pedido_idPedido inner join detallepedidobebida dpb on dpb.pedido_idPedido = p.idPedido inner join productos pro on pro.idProducto = dpb.productos_idProducto where p.idPedido="+Convert.ToUInt32(txt_noPedido.Text)+" ", con.conexion());
                dr = com.ExecuteReader();
                while (dr.Read())
                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string fecha = null;
            string mesa = null;
            com = new OdbcCommand("select concat(c.nombreCliente,' ', c.apellidoCliente), p.fechaPedido, p.mesa_idMesa from cliente c inner join pedido p on p.cliente_idCliente=c.idCliente where idPedido=" + Convert.ToInt32(this.id_pedido) + " ", con.conexion());
            dr = com.ExecuteReader();
            while (dr.Read())
            {
            ...
            com = new OdbcCommand("select fechaPedido from pedido where idPedido=" + Convert.ToInt32(this.id_pedido) + " ", con.conexion());
            dr = com.ExecuteReader();
            while (dr.Read())
            {
            ...
            com = new OdbcCommand("select mesa_idMesa from pedido where idPedido=" + Convert.ToInt32(this.id_pedido) + " ", con.conexion());
            dr = com.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void btn_editar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                com = new OdbcCommand("update pedido set fechaPedido='" + dtp_fecha.Value.ToString("d/M/yyyy") + "', mesa_idMesa=" + Convert.ToInt32(cmb_mesa.Text) + ", cliente_idCliente=" + id_cliente + " where idPedido="+ Convert.ToInt32(this.id_pedido)+" ", con.conexion());
                com.ExecuteNonQuery();
        ...
        private void btn_borrar_Click(object sender, EventArgs e)
        {
            try
            {
                com = new OdbcCommand("delete from pedido where idPedido=" + Convert.ToInt32(this.id_pedido) + " ", con.conexion());
                com.ExecuteNonQuery();
                mostrar_pedido();
                cmb_cliente.Text = "SELECCIONE OPCION";
                cmb_mesa.Text = "SELECCIONE OPCION";
